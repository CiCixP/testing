...
TrilinearInterpolation::getCornerIndices(
    const std::vector<Real> & v, Real x, int & lower, int & upper, Real & d)
{
...
Real
TrilinearInterpolation::getCornerValues(int x, int y, int z)
{
  int nY = _y_axis.size();
  int nZ = _z_axis.size();
...
Real
TrilinearInterpolation::sample(Real x, Real y, Real z)
{
  int x0 = 0;
  int y0 = 0;
  int z0 = 0;
